...
export default function UpdateUserCo({ id }) {
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <Grid container spacing={3} >
        ...
        <Grid item xs={12} md={6} className='flex !flex-col'>
          <SelectRoleCo defaultValue={user?.roleId}/>
        </Grid>
        <Grid item xs={6} md={3} className='flex !flex-col'>
          <Button type='submit' variant='contained' className='bg-primary'>Update</Button>
        </Grid>
        <Grid item xs={6} md={3} className='flex !flex-col'>
          <Button href='/coordinator/user' type='button' variant='contained' color='error' className='bg-error'>Cancel</Button>
        </Grid>
      </Grid>
    </form>
  )
